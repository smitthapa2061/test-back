...
const computeOverallMatchDataForRound = async (tournamentId, roundId, matchId, userId) => {
  ...
  // Aggregate all previous matches for real-time updates
...
const getOverallMatchDataForRound = async (req, res) => {
  try {
    ...
    // Emit overall data via socket for real-time updates
    try {
      const io = getSocket();
      io.emit('overallDataUpdate', { tournamentId, roundId, matchId, teams: aggregatedTeams, createdAt: new Date() });
    } catch (socketError) {
